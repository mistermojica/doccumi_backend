    ...
    (async () => {
      ...
      }
      {
          ...
          page.waitForXPath(`//span[contains(text(),'${ctx.identificador}')]`).then((resSelector) => {
            console.log('Encontró el identificador:', ctx.identificador);
            browser.close();
            resolve({success: true, message: "Archivo publicado exitosamente.", result: null});
          }).catch(() => {
            console.log('No encontró el identificador:', ctx.identificador);
            reject({success: false, message: "Archivo no pudo ser publicado.", result: null});
          });
      }
    })();
  ...
  var promise = new Promise(function(resolve, reject) {
    (async () => {
      ...
      page.waitForXPath("//div[contains(text(),'Post shared')]").then((resSelector) => {
        console.log('Encontró el identificador de publicación.');
        browser.close();
        resolve({success: true, message: "Archivo publicado exitosamente.", result: null});
      }).catch(() => {
        console.log('No encontró el identificador de publicación.');
        reject({success: false, message: "Archivo no pudo ser publicado.", result: null});
      });
    })();
